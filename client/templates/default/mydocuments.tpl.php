...
?>
							...
							<div class="tituloDropdown"><?php echo $trans->getTrans($_REQUEST["action"],'COLLECTION'); ?>: <b><?php if ( !$resultDirName ) { echo $trans->getTrans($_REQUEST["action"],'INCOMING_FOLDER'); } ?><?php echo $resultDirName; ?></b>
								<?php if ( $resultDirName ) : ?>
									...
									<ul id='dropdown1' class='dropdown-content'>
										...
										<li><a href="#modal3" class="modal-trigger" onclick="__gaTracker('send','event','Favorite Documents','Share Collection','<?php echo htmlspecialchars($resultDirName); ?>');"><i class="fas fa-share-alt right m1"></i><?php echo $trans->getTrans($_REQUEST["action"],'SHARE_COLLECTION'); ?></a></li>
									</ul>
								<?php endif; ?>
							</div>
						</div>
				...
				<section class="row">
					...
					<?php else : ?>
                        <div class="card-panel center-align">
   							<span class="blue-text text-darken-2"><?=$trans->getTrans($_REQUEST["action"],'ACCESS_LIST_NO_REGISTERS_FOUND')?></span>
  						</div>
                    <?php endif; ?>
				</section>
				<?php
                    if ( $objPaginator->totalPages > 1 ) {
                        echo $objPaginator->build();
                    }
                ?>
			</div>
		</div>
	...
	<div id="modal3" class="modal">
		<form action="#" class="col s12">
			...
			<div class="modal-footer">
				<a href="#!" class="btn btnDanger modal-close">Cancelar</a>
			</div>
		</form>
	</div>
	<!-- Modal Similares -->
	<div id="modal-related-docs" class="modal">
		<form action="#" class="col s12">
			...
			<div class="modal-footer">
				<a href="#!" class="btn btnDanger modal-close">Fechar</a>
			</div>
		</form>
	</div>
	<!-- Modal Excluir Item -->
