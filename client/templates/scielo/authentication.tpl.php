...
<? $b64HttpHost = base64_encode($_SERVER["HTTP_HOST"]); ?>
    ...
    <?if ($response['values']["status"] === false){?>
        <?if (preg_match(REGEXP_EMAIL,$_REQUEST["userID"])){?>
            <div class="alert"><?=str_replace('HELP_FORM','<a href="'.SERVICES_PLATFORM_DOMAIN.'/pub/contact.php?c='.$b64HttpHost.'" target="_parent">'.$trans->getTrans($_REQUEST["action"],'PRESS_HERE').'</a>',$trans->getTrans($_REQUEST["action"],'INVALID_LOGIN_MAIL'))?></div>
        <?}else{?>
            <div class="alert"><?=str_replace('HELP_FORM','<a href="'.SERVICES_PLATFORM_DOMAIN.'/pub/contact.php?c='.$b64HttpHost.'" target="_parent">'.$trans->getTrans($_REQUEST["action"],'PRESS_HERE').'</a>',$trans->getTrans($_REQUEST["action"],'INVALID_LOGIN_UNAME'))?></div>
        <?}?>
    <?}?>
