...
<div class="login_btn">
    ...
    <?}else{?>
        <a href="<?=RELATIVE_PATH?>/controller/authentication/origin/<?=$_REQUEST["origin"]?>" target="_parent"><span class="btn_l"><?=$trans->getTrans($_REQUEST["action"],'LOGIN')?></span><span class="btn_r"><?=$trans->getTrans($_REQUEST["action"],'FOR_SERVICES')?></span></a>
    <?}?>
</div>
