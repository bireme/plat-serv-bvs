        ...
        <?if ($response["values"] != false ){?>
            <ul>
                ...
                <?foreach ($response["values"] as $register){
                     $count++;?>
                    <li><a href="<?=RELATIVE_PATH?>/controller/myprofiledocuments/control/business/profile/<?=$register["profileID"]?>/mode/<?=$_REQUEST["mode"]?>"><?=$register["profileName"]?></a><?if ($register["profileDefault"] == 1){?>&nbsp;<img src="<?=RELATIVE_PATH?>/images/<?=$_SESSION["skin"]?>/page_bookmark.gif" border="0"/><?}?></li>
                <?}?>
                </div>
            </ul>
        <?}?>
