...
<div class="form">
    ...
    <form name="form" method="post" action="<?=RELATIVE_PATH?>/controller/directories">
        ...
        <table class="form" cellspacing="0">
            <tr>
                <td>
                    <input type="radio" name="directory" value="0" checked="true"><label for="folderCheck"><img src="<?=RELATIVE_PATH?>/images/<?=$_SESSION["skin"]?>/folder.gif" /><?=$trans->getTrans($_REQUEST["action"],'INCOMING_FOLDER')?></label>
                </td>
            </tr>
            ...
            <tr>
                <td>
                    ...
                    <input type="submit" value="<?=$trans->getTrans($_REQUEST["action"],'CANCEL')?>" class="submitFalse" onClick="window.close(); " />
                </td>
            </tr>
        </table>
    </form>
