...
class Log_file extends Log
{
    ...
    function open()
    {
        if (!$this->_opened) {
			...
			/* If the file isnt "opened", open a temp file (*nix like OS)
			 * @author gustavo.fonseca@bireme.org
			 */
			 if(!$this->_fp){
				 //$tmpFile = '/tmp/servPlatLog.out';
			     $this->_fp = fopen(ALT_LOG_FILE, ($this->_append) ? 'a' : 'w');
			 }
